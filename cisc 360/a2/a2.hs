...
rewrite important []       = ""
rewrite important (c : cs) = if important c then [c] ++ [c] ++ (rewrite important cs) else [c] ++ rewrite important cs
...
listCompare1 :: ([Integer], [Integer]) -> [Bool]
listCompare1 (xs, ys) = zipWith (==) xs ys
{-
  Q3b.
  ...
  If you were to use this implementation, it would only ever work with non empty lists, implying that
    both lists would need to be the same length. Without this requirement, it will simply run until one of the lists is empty,
    and because you wouldn't have both xs and ys, rather you'd have xs and [] or [] and ys, therefore it would terminate.
...
-}
sing :: Song -> Song
sing (Harmony (Harmony (Melody n) motif) right) = if n /= 0 then motif else (Harmony (Harmony (Melody n) motif) right)
...
s1 = Harmony (Melody 0) (Harmony (Melody 0) (Melody 1))
s2 = Melody 1
s3 = Harmony (Harmony (Melody 0) (Harmony (Melody 0) (Harmony (Melody 0) (Melody 1)))) (s1)

total = Harmony (Harmony (Harmony (Melody 0) (s1)) (s1)) (s3)
diverging_song = total
-- holy heck i actually did it
