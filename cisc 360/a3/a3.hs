...
getValuations []       = [[]]
getValuations c = sequence (map (\s -> [(s,True), (s,False)]) c)
...
evalF _    Top                 = True
evalF _    Bot                 = False
evalF valu (Not phi1)          = if (evalF valu phi1) then False else True
evalF valu (Atom c)            = if (find (\x -> x == (c, True)) valu /= Nothing) then True else False 
...
evalF valu (Implies phi1 phi2) = if (not (evalF valu phi1)) || (evalF valu phi2) then True else False
evalF valu (Equiv phi1 phi2)   = if (evalF valu (Implies phi1 phi2)) && (evalF valu (Implies phi2 phi1)) then True else False
